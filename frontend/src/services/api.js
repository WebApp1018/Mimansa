...
export const restApiSettings = {
  baseURL: process.env.REACT_APP_API_BASE_URL || 'http://54.165.194.118:8000/api',
...
export const backendSettings = {
  ...
  logoBaseURL: process.env.REACT_APP_API_BASE_URL || 'http://anuragportmap-44814.portmap.host:56665/logo',
}
...
export async function apiValidateDockDoor(searchParams) {
  return await query(`/dock_door`, { searchParams: searchParams, method: 'POST' });
}
export async function apiValidateTrailer(searchParams) {
  return await query(`/trailer`, { searchParams: searchParams, method: 'POST' });
}
export async function apiValidateLoadCarton(searchParams) {
  return await query(`/load_carton`, { searchParams: searchParams, method: 'POST' });
}
export async function apiUpdateCartonBoxCount(searchParams) {
  return await query(`/update_carton_box_count`, { searchParams: searchParams, method: 'POST' });
}